...
export function DashboardView({ data }: { readonly data: AppData; readonly setData: (data: AppData) => void }) {
  ...
  const recentTransactions = recentTransactionRows(data.transactions);
...
function KpiCard(props: { readonly label: string; readonly value: string; readonly tone?: "success" | "danger" }) {
  const toneClass = props.tone === "success" ? "text-[var(--color-success)]" : props.tone === "danger" ? "text-[var(--color-error)]" : "text-[var(--color-text)]";
  return (
    <div className="panel p-4">
      <div className="text-sm text-[var(--color-text-secondary)]">{props.label}</div>
  ...
  return (
    <div className="panel p-4">
      <div className="flex items-center justify-between gap-3">
        <div className="text-sm text-[var(--color-text-secondary)]">本月收支</div>
        <div className="rounded bg-[var(--color-surface-muted)] px-2 py-0.5 text-xs font-medium text-[var(--color-text-secondary)]">{props.row.currency}</div>
      </div>
      <div className="mt-3 grid gap-2 text-sm">
        <div className="flex items-center justify-between gap-3">
          <span className="text-[var(--color-text-secondary)]">收入</span>
          <span className="font-semibold text-[var(--color-success)]">{money(props.row.income, props.row.currency)}</span>
        </div>
        <div className="flex items-center justify-between gap-3">
          <span className="text-[var(--color-text-secondary)]">支出</span>
          <span className="font-semibold text-[var(--color-error)]">{money(props.row.expense, props.row.currency)}</span>
        </div>
      </div>
    </div>
  );
...
function TransactionLine(props: { readonly row: Transaction }) {
  return (
    <div className="grid grid-cols-[1fr_auto] gap-3 px-4 py-3 text-sm">
      <div className="min-w-0">
        <div className="truncate font-medium">{props.row.note || TRANSACTION_KIND_LABELS[props.row.kind]}</div>
        <div className="mt-1 text-xs text-[var(--color-text-secondary)]">{dateOnly(props.row.occurredAt)} · {TRANSACTION_KIND_LABELS[props.row.kind]}</div>
      </div>
      ...
      <div className="divide-y divide-[var(--color-border)]">
        ...
        {props.rules.map((rule) => (
          <div key={rule.id} className="px-4 py-3 text-sm">
            ...
            <div className="mt-1 text-xs text-[var(--color-text-secondary)]">下次：{new Date(rule.nextRunAt).toLocaleDateString("zh-CN")}</div>
          </div>
        ))}
      </div>
    </section>
...
function BudgetPanel({ data, entries }: { readonly data: AppData; readonly entries: readonly ReportEntry[] }) {
  return (
    <section className="panel p-4">
      ...
      {data.budgets.length === 0 && <p className="mt-3 text-sm text-[var(--color-text-secondary)]">暂无预算。</p>}
...
function BudgetProgress(props: { readonly budget: Budget; readonly entries: readonly ReportEntry[] }) {
  ...
  return (
    <div className="row-card p-3 text-sm">
      <div className="flex items-center justify-between gap-3">
        <span className="truncate font-medium">{props.budget.name}</span>
        <span className="text-[var(--color-text-secondary)]">{percent}%</span>
      </div>
      <div className="mt-3 h-2 overflow-hidden rounded bg-[var(--color-surface)]">
        <div className="h-full rounded bg-[var(--color-accent)]" style={{ width: `${percent}%` }} />
      </div>
      <div className="mt-2 flex justify-between gap-3 text-xs text-[var(--color-text-secondary)]">
...
function PanelTitle(props: { readonly title: string; readonly compact?: boolean }) {
  return <h2 className={props.compact ? "font-semibold" : "border-b border-[var(--color-border)] px-4 py-3 font-semibold"}>{props.title}</h2>;
}

function EmptyLine(props: { readonly text: string }) {
  return <p className="px-4 py-6 text-sm text-[var(--color-text-secondary)]">{props.text}</p>;
}
