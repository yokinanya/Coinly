    ...
    <section className="space-y-5">
      ...
      <div className="grid gap-4 lg:grid-cols-2">
        ...
        <div className="panel p-4">
          <h2 className="font-semibold text-(--color-text)">订阅规则</h2>
          <div className="mt-3 space-y-2">
            {pendingRecurring.length === 0 && <p className="text-sm text-(--color-text-secondary)">暂无启用的订阅。</p>}
            {pendingRecurring.map((rule) => (
              <div key={rule.id} className="row-card p-3 text-sm">
                <div className="font-medium">{rule.name}</div>
                <div className="text-(--color-text-secondary)">下次：{new Date(rule.nextRunAt).toLocaleDateString("zh-CN")}</div>
              </div>
            ))}
          </div>
        </div>
  ...
  return (
    <div className="panel p-4">
      ...
      <div className="mt-3 space-y-3">
        {props.rows.length === 0 && <p className="text-sm text-(--color-text-secondary)">暂无数据。</p>}
        {props.rows.map((row) => (
